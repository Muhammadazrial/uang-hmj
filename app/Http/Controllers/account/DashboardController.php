...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $uang_masuk_bulan_ini  = DB::table('pemasukan')
        ...
        $uang_keluar_bulan_ini = DB::table('pengeluaran')
            ->selectRaw('sum(nominal) as nominal')
        ...
        $uang_masuk_bulan_lalu  = DB::table('pemasukan')
            ->selectRaw('sum(nominal) as nominal')
            ->whereYear('debit_date', Carbon::now()->year)
        ...
        $uang_keluar_bulan_lalu = DB::table('pengeluaran')
            ->selectRaw('sum(nominal) as nominal')
            ->whereYear('credit_date', Carbon::now()->year)
        ...
        $uang_masuk_selama_ini  = DB::table('pemasukan')
            ->selectRaw('sum(nominal) as nominal')
            ->where('user_id', Auth::user()->id)
            ->first();

        $uang_keluar_selama_ini = DB::table('pengeluaran')
            ->selectRaw('sum(nominal) as nominal')
            ->where('user_id', Auth::user()->id)
            ->first();
